...
Auf dem UML Diagramm ist eine rekursive Traversierungsfunktion zu sehen.
Diese wird hier genauer erklärt und mit einem weiteren Aktivitätsdiagramm veranschaulicht.
...
Die Methode \texttt{traverse\_game\_tree} realisiert die rekursive CFR-Traversierung für einen Spieler.
Sie berechnet den erwarteten Nutzen entlang des Baums und aktualisiert an jedem \emph{Spieler-Knoten} die kumulierten Regrets sowie die kumulierte Strategie für das zugehörige Informationsset.
An Zufalls- und Gegner-Knoten wird nur der Nutzen propagiert; \texttt{reach\_probabilities} und \texttt{chance\_reach} entsprechen den Erreichbarkeitswahrscheinlichkeiten $\pi_i^\sigma(h)$ bzw.\ der kumulierten Zufallswahrscheinlichkeit und werden für die Gewichtung in \texttt{update\_regrets} und \texttt{update\_strategy\_sum} benötigt.
\newpage
Es folgt Quellcode der rekursiven Traversierungsfunktion des dynamischen CFR Solvers:

\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize, caption={\texttt{traverse\_game\_tree}}, breaklines=true]
...
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize, caption={\texttt{update\_regrets} und \texttt{update\_strategy\_sum}}, breaklines=true]
...
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\footnotesize, caption={Regret Matching: \texttt{\_compute\_policy\_for\_infoset}}, breaklines=true]
...
Auf der folgenden Seite ist die Layer-basierte Traversierung in einem UML-Aktivitätsdiagramm dargestellt.
