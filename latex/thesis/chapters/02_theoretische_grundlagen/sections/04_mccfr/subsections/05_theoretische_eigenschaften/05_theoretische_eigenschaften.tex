...
Diese Schranke verbessert die ursprüngliche Schranke aus Theorem 4 des Zinkevich Papers, die $|\mathcal{I}_i|$ anstelle von $M_i$ verwendet.
...
Für Outcome Sampling MCCFR gilt mit Wahrscheinlichkeit mindestens $1-p$ für $p \in (0, 1]$:
\begin{equation}
R_i^T \le \left(1 + \sqrt{\frac{2}{p}} \cdot \frac{1}{\sqrt{\delta}}\right) \Delta_{u,i} M_i \sqrt{|A_i|}/\sqrt{T},
\end{equation}
wenn $q(z) \ge \delta > 0$ für alle relevanten terminal histories $z$ gilt (\cite[Thm.~5]{lanctot2009monte}).
Im Vergleich zu External Sampling enthält diese Schranke den zusätzlichen Faktor $\frac{1}{\sqrt{\delta}}$, der von der sampling policy abhängt.
