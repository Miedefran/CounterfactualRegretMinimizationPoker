...
def main():
    ...
    if args.game == 'kuhn':
        ...
        game = TwelveCardPokerGame(ante=1, bet_sizes=[2, 4, 8], bet_limit=2)
        print(f"Initialized Twelve Card Poker game")
    elif args.game == 'rhode_island':
        game = RhodeIslandGame(ante=5, bet_sizes=[10, 20, 20], bet_limit=3)
        print(f"Initialized Rhode Island Hold'em game")
    elif args.game == 'royal_holdem':
        game = RoyalHoldemGame(ante=1, bet_sizes=[2, 4, 8, 8], bet_limit=3)
