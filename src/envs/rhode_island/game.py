...
from utils.poker_utils import RhodeIslandCombinations


class RhodeIslandGame(LeducHoldemGame):
    combination_generator = RhodeIslandCombinations()
    ...
    def __init__(self):
        ...
        self.round = RhodeIslandRound(bet_sizes=self._bet_sizes, bet_limit=self._bet_limit)
        self.public_cards = []

    def reset(self, starting_player):
        super().reset(starting_player)
        self.public_cards = []
        # Round init happens after private deal chance resolves (see Leduc reset)
