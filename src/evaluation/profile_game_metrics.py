"""
Profiling-Skript für Kapitel 6:

Schreibt pro Spiel eine kleine Text-Zusammenfassung:
- #Nodes und #Infosets des Game Trees (falls vorhanden)
- #States des Public State Trees (falls vorhanden)
- Dauer einer Best-Response-Berechnung für EINEN Spieler (player_id)

Hinweis:
- Dieses Skript baut Trees NICHT automatisch (damit es nicht aus Versehen Stunden läuft).
- Es erwartet i.d.R. eine existierende Strategie-Datei (*.pkl.gz), um BR realistisch zu messen.
"""
...
def main() -> None:
    ...
    parser.add_argument("--strategy", required=True, type=str, help="Pfad zu einer *.pkl.gz Model-Datei (mit average_strategy)")
    parser.add_argument("--player", type=int, default=0, choices=[0, 1], help="Spieler für BR-Zeitmessung (default: 0)")
    parser.add_argument("--output", type=str, default=None, help="Optionaler Output-Pfad (*.txt). Wenn nicht gesetzt: nur stdout.")
    parser.add_argument(
        "--abstract-suits",
        action="store_true",
